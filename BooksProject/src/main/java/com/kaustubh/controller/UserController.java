...
import com.kaustubh.validator.UserValidator;
...
public class UserController {
	...
	protected String loginUser(HttpServletRequest request) throws Exception {
		...
		try {
			...
			if(u.getStatus().equalsIgnoreCase("Publisher")){
				return "user-home";
			}
			
			else if(u.getStatus().equalsIgnoreCase("Customer")){
	...
	protected ModelAndView registerNewUser(HttpServletRequest request,  @ModelAttribute("user") User user, BindingResult result) throws Exception {
		...
		try {
			...
			if(u.getStatus().equalsIgnoreCase("customer")){
				return new ModelAndView("customer-home","user", u);
			}
			else if(u.getStatus().equalsIgnoreCase("publisher")){
